...
#else
#define BASE_FREQUENCY 				901500000		/**< P2P basic frequency. Change this define to work in another frequency. */
#endif
...
#ifdef MASTER_DEVICE
#define MY_ADDRESS                  MASTER_ADDRESS 				/* < This device address */
#define DESTINATION_ADDRESS         BROADCAST_ADDRESS 			/* < Change this to send messages to another device */
#else
#define MY_ADDRESS                  0x02		/* < This device address */
#define DESTINATION_ADDRESS         MASTER_ADDRESS 			/* < Change this to send messages to another device */
#endif
...
//#define PAYLOAD_LEN                     25 					/* < *16 bytes data+tag+cmd_type+cmd+cmdlen+datalen+device_id*/
...
typedef struct {
	...
	uint8_t battery_lvl;
	uint8_t device_id;
} __attribute__((packed)) AppliFrame_t;
...
/*!******************************************************************
 * \fn void AppliSendBuff(AppliFrame_t *xTxFrame, uint8_t dst_addr)
...
/*!******************************************************************
 * \fn void P2P_Process(uint8_t *pTxBuff)
...
/*!******************************************************************
 * \fn void P2P_SendData(uint8_t *pTxBuff)
